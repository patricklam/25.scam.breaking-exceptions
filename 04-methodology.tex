The previous section, Section~\ref{sec:data-collection}, described how we collected both the client programs and the necessary information to perform our analysis.
...
As described in Section~\ref{sec:data-collection}, we begin by selecting Java-based clients from the DUETS dataset~\cite{durieux21:_duets}. We retain only those clients that are Maven-compatible and successfully compile to a JAR file, ensuring compatibility with our analysis tooling.\todo{do we retain all such clients or do we throw any out for any reason? - we retain all clients that compile to a jar}
...
To enable taint analysis, we automatically generate a \textit{driver stub} for each library method used by the client\todo{let's improve this. first, why do we want to do taint analysis at all? think about how well what we've explained so far justifies that. do we have enough examples about it, and should we have another mini-example in this section? We have an example below, maybe we move it up and talk about how tainting works here, if it's appropriate. second, why do we need to generate a driver stub anyway? why can't we just use taint analysis?}. The stub declares each method parameter as a taint source. For each parameter, we define a wrapper function and mark its return value as a FlowDroid source. Stub generation, implemented using SootUp, handles a variety of cases, including:
\begin{itemize}
