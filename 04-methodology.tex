...
\todo{this subsubsection is completely repeated from section 3, isn't it? I think it should all be in section 3 and none here}
...
\todo{We should include an example of a driver stub, probably in the motivating example.}
...
After collecting the list of unchecked exceptions for each method in both the old and latest versions of the library, we perform a comparison to identify newly introduced exceptions. If a method exists in the old version but is missing from the latest version, we exclude it from our analysis, as its removal may indicate a method signature–based breaking change, which is outside the scope of our detection\todo{Maybe, we write something about JAPICMP?}.
...
For cases where we cannot generate a test case due to hardcoded arguments or guarding conditions, the exception is not currently triggerable. However, the presence of a newly added exception remains valuable information for the client developer, as future changes (e.g., modifying the check or input value) could expose the call site to a runtime failure.\todo{repeating ourselves}
