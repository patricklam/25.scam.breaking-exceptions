...
For technical reasons related to FlowDroid, we automatically generate a \textit{driver stub} for each value that the client supplies to the library. FlowDroid does not allow method parameters to be marked directly as taint sources. To work around this, we wrap each parameter in a synthetic method and mark its return value as a source. In our analysis, we treat the parameters of library methods invoked by the client as taint sources, since these are the only values under the client’s control. We are specifically interested in whether these client-supplied values can trigger newly introduced exceptions. If not, then the exception is effectively unreachable from the client, and thus does not constitute a behavioural breaking change. Finally, we treat each exception identified in the previous step as a potential taint sink.
