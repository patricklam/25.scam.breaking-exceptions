...
To begin our analysis, we first collected suitable client projects\todo{you said that there was a huge number of clients in DUETS. Presumably more than the number we have are 5-starred. Need to say something about how we selected a subset to analyze, even if it's a convenience sample.}. We used the DUETS dataset~\cite{durieux21:_duets}, which provides a curated list of Java-based clients hosted on GitHub, each with at least five stars. We attempted to download each client repository and discarded any client that failed to download.
...
\todo{similarly: a client presumably has many libraries that it depends on; are we using the library that DUETS pairs with the client? We should say so.}
