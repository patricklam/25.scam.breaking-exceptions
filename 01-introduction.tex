...
\todo{we want the number of libraries that add new exceptions without reference to callsites first. I don't think this is anywhere in the paper and should be a separate result. Then we talk about how many of them occur in client-library pairs as in the paper}
[The first sentence actually should answer the question about libraries without reference to clients, beccause that is actually what RQ1 is about]. % , sometimes in minor and patch version upgrades
...
\todo{there should be a figure that illustrates the whole client-library upgrade flow, as in the developer-relevant thing, in addition to figure 1, which is the methodology-relevant thing.}


The contributions of this work are as follows:

\begin{itemize}[noitemsep]
\item We conduct an empirical study of real-world client-library pairs to assess the prevalence of behavioural breaking changes caused by newly added unchecked exceptions.
\item We introduce \textit{UnCheckGuard}, a static analysis tool that combines RTA-based call graph construction with taint analysis to detect and verify newly added unchecked exceptions.
\item We evaluate 98 client-library pairs from the DUETS dataset~\cite{durieux21:_duets} using \textit{UnCheckGuard}, identifying 14 call sites where newly added unchecked exceptions could cause behavioural breaking changes.
\end{itemize}
