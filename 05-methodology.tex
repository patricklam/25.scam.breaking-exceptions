...
We divide the analysis setup into two steps: 1) collecting the current and latest library versions, and 2) mapping external method invocations to library methods. Figure~\ref{fig:jsonjava} shows an overview of the full pipeline.
...
After selecting clients and compiling them into JAR files, we collect the current and latest versions of the libraries on which the client depends.
