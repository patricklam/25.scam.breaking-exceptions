The previous section, Section~\ref{sec:data-collection}, described how we collected the clients.
...
We divide the analysis setup into the following steps: collecting the current and latest library versions, and mapping external method invocations to library methods. Figure~\ref{fig:jsonjava} shows an overview of the full pipeline.
...
After selecting the valid clients and compiling them into JAR files, we collect the current and latest versions of the libraries on which the client depends.
...
After acquiring both the current and latest versions of the libraries on which the client depends, we extract all external method invocations made by the client.
...
We mark an exception site as \emph{reachable} if either the client-supplied value is used as an argument for the exception constructor or it influences the control flow that reaches the \texttt{throw} statement. This approach ensures that UnCheckGuard correctly identifies both explicit data dependence and implicit control-flow dependence on the client-supplied value.

Figure~\ref{fig:taint-flow} illustrates the two ways in which client-supplied values can reach an exception site in our analysis: directly through constructor arguments (data flow) or indirectly by influencing the control flow that leads to the \texttt{throw} statement.
