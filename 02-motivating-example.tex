...
version 4.4.6 of the \texttt{httpcore} library\todo{that is true, right?}. Since the release of DUETS, the \texttt{httpcore} developers
...
To detect this change, UnCheckGuard processes JAR files for both \texttt{httpcore-4.4.6} and \texttt{httpcore-4.4.16}, along with the entry method\todo{why do we say this is an entry method? that is, who indicates the entry methods?} used by the client---here, \texttt{createAsyncClient(boolean)}\footnote{Specifically, method \texttt{createAsyncClient(boolean)} returning a \texttt{CloseableHttpAsyncClient} on class \texttt{Util.HttpClientUtil.HttpAsyncClient}}. It uses SootUp~\cite{Karakaya24:_sootup} to construct an RTA-based call graph~\cite{bacon96:_fast_static_analy_c_virtual_funct_calls} starting from the entry method\todo{is there just one entry method, or several?} and identifies all transitively reachable methods. UnCheckGuard then collects all unchecked exceptions thrown within this graph and applies taint analysis using FlowDroid~\cite{Arzt14:_flowdroid} to check whether method parameters controlled by the client can reach the exception sites. In version 4.4.6, it finds two sites throwing \texttt{IllegalArgumentException}, while in 4.4.16, it detects three—each of which the client can potentially trigger using the values it chooses to pass to the library as parameters.
...
public \texttt{createAsyncClient} method takes a \texttt{proxy}
...
To detect that our \texttt{HttpAsyncClientUtils} client calls a method from \texttt{httpcore-4.4.6} which, upon upgrading to \texttt{httpcore-4.4.16}, may throw a new unchecked exception, UnCheckGuard begins by identifying all library methods invoked by the client\todo{starting at which entry points?}. It then analyzes both the existing and the upgraded versions of the library. Using this analysis, it determines whether any newly introduced unchecked exceptions are reachable from the client's code. Here, reachability means
that the client can trigger the exception on some execution of the program, using values it passes as parameters,
which implies that a path exists in the interprocedural control-flow graph, computed with RTA, from the client method to the exception-throwing site in the library.\todo{there are two conditions here; the second one is weaker than the first one. Which do we check? Or is it a third thing, ie what FlowDroid computes?}
