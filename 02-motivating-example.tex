...
version 4.4.6 of the \texttt{httpcore} library\todo{that is true, right? - Yes}. Since the release of DUETS, the \texttt{httpcore} developers
...
To detect that our \texttt{HttpAsyncClientUtils} client calls a method from \texttt{httpcore-4.4.6} which, upon upgrading to \texttt{httpcore-4.4.16}, may throw a new unchecked exception, UnCheckGuard begins by identifying all library methods invoked by the client\todo{starting at which entry points? - we make a sootup view and extract all InvokeStmt}. It then analyzes both the existing and the upgraded versions of the library. Using this analysis, it determines whether any newly introduced unchecked exceptions are reachable from the client's code. Here, reachability means
that the client can trigger the exception on some execution of the program, using values it passes as parameters,
which implies that a path exists in the interprocedural control-flow graph, computed with Class Hierarchy Analysis (CHA), from the client method to the exception-throwing site in the library.\todo{there are two conditions here; the second one is weaker than the first one. Which do we check? Or is it a third thing, ie what FlowDroid computes?}
